...
const fixtures = [
  ...
  // OI-82 — lib.mjs's schema engine. Eight keyword branches that no shipped schema had ever
  // violated, so none had run: the repo's own configs are valid, which is exactly the condition
  // that leaves a validation engine untested. All eight go through one probe schema in a fixture
  // definitions root, with one config per keyword violating only that keyword.
  { id: 'ho', dir: 'test/fixtures/lifecycle-violations/ho-schema-pattern', description: '(OI-82) a value fails its schema pattern — lib.mjs schema engine', validator: validatorPath('check-config.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/schema-keyword-probe' }, expect: 'does not match pattern' },
  { id: 'hp', dir: 'test/fixtures/lifecycle-violations/hp-schema-minlength', description: '(OI-82) a string is shorter than minLength — lib.mjs schema engine', validator: validatorPath('check-config.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/schema-keyword-probe' }, expect: 'is shorter than minLength 5' },
  { id: 'hq', dir: 'test/fixtures/lifecycle-violations/hq-schema-minimum', description: '(OI-82) a number is below minimum — lib.mjs schema engine', validator: validatorPath('check-config.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/schema-keyword-probe' }, expect: 'is below minimum 10' },
  { id: 'hr', dir: 'test/fixtures/lifecycle-violations/hr-schema-maximum', description: '(OI-82) a number is above maximum — lib.mjs schema engine', validator: validatorPath('check-config.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/schema-keyword-probe' }, expect: 'is above maximum 10' },
  { id: 'hs', dir: 'test/fixtures/lifecycle-violations/hs-schema-minitems', description: '(OI-82) an array has fewer than minItems — lib.mjs schema engine', validator: validatorPath('check-config.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/schema-keyword-probe' }, expect: 'has fewer than 2 item(s)' },
  ...
  // WP-R18 — check-lifecycle's provenance manifest rules (4 of 4). Each asserts THAT rule's own
  // wording rather than a shared substring, so a fixture cannot pass while leaving its rule
  // undefended — the failure mode test/run-mutation-audit.mjs's header records three times.
  { id: 'pv1', dir: 'test/fixtures/definitions/pv-provenance-wrong-kind', description: '(WP-R18) a provenance manifest declares the wrong kind — check-lifecycle', validator: validatorPath('check-lifecycle.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/pv-provenance-wrong-kind' }, expect: 'has kind "manifest", expected "provenance"' },
  ...
  { id: 'pv6', dir: 'test/fixtures/definitions/pv-provenance-all-drifted', description: '(WP-R18) every recorded CONFIG digest differs from disk — the signature of a post-setup baseline never re-taken. Scoped to config/ because setup rewrites those five and never touches the hook or the adapters, so a rule keyed on the whole governed set could not fire in the case it targets', validator: validatorPath('check-lifecycle.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/pv-provenance-all-drifted' }, expect: 'EVERY one differs from what is on disk' },
  ...
  { id: 'ij', dir: 'test/fixtures/definitions/sk-unsupported-keyword', description: '(OI-82) a schema uses a keyword the engine does not implement — check-schema-keywords', validator: validatorPath('check-schema-keywords.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/sk-unsupported-keyword' }, expect: 'uses "multipleOf", which validateSchema does not implement' },
  { id: 'ik', dir: 'test/fixtures/definitions/sk-positional-misplaced', description: '(OI-82) x_enforcement sits outside the schema position that honours it — check-schema-keywords', validator: validatorPath('check-schema-keywords.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/sk-positional-misplaced' }, expect: 'uses "x_enforcement" outside a schema-valued' },
  // OI-82 — check-starter-blocks (5 of 5). Four go through --file, which takes a single
  // output-schema.md and skips the contract comparison. The fifth needs the contract, so it runs in
  // default mode against a definitions root holding all seven starter blocks with one artifact line
  // diverged — and the divergence has to be made INSIDE the fenced block, since these files mention
  // `artifact:` in prose above it too.
  { id: 'il', dir: 'test/fixtures/lifecycle-violations/if-starter-no-section', description: '(OI-82) an output schema has no Starter Block section — check-starter-blocks', validator: validatorPath('check-starter-blocks.mjs'), args: ['--file', 'test/fixtures/lifecycle-violations/if-starter-no-section/output-schema.md'], expect: 'is missing a "## Starter Block" section' },
  { id: 'im', dir: 'test/fixtures/lifecycle-violations/ig-starter-no-fence', description: '(OI-82) a Starter Block heading carries no fenced block — check-starter-blocks', validator: validatorPath('check-starter-blocks.mjs'), args: ['--file', 'test/fixtures/lifecycle-violations/ig-starter-no-fence/output-schema.md'], expect: 'no ```markdown fenced block' },
  { id: 'in', dir: 'test/fixtures/lifecycle-violations/ih-starter-bad-frontmatter', description: '(OI-82) a starter block\'s frontmatter does not parse — check-starter-blocks', validator: validatorPath('check-starter-blocks.mjs'), args: ['--file', 'test/fixtures/lifecycle-violations/ih-starter-bad-frontmatter/output-schema.md'], expect: 'starter block frontmatter does not parse' },
  { id: 'io', dir: 'test/fixtures/lifecycle-violations/ih-starter-bad-frontmatter', description: '(OI-82) a named starter block file does not exist — check-starter-blocks', validator: validatorPath('check-starter-blocks.mjs'), args: ['--file', 'test/fixtures/lifecycle-violations/absent-by-design/output-schema.md'], expect: 'absent-by-design/output-schema.md is missing' },
  { id: 'ip', dir: 'test/fixtures/definitions/sb-artifact-mismatch', description: '(OI-82) a starter block declares an artifact its contract does not — check-starter-blocks', validator: validatorPath('check-starter-blocks.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/sb-artifact-mismatch' }, expect: 'starter block artifact "plan" != contract "brief"' },
  // OI-82 — check-scope-fence (4 of 5).
  { id: 'iq', dir: 'test/fixtures/lifecycle-violations/iq-scope-interior-glob', description: '(OI-82) a plan Touches entry carries an interior glob the fence cannot expand — check-scope-fence', validator: validatorPath('check-scope-fence.mjs'), expect: 'contains an interior glob' },
  { id: 'ir', dir: 'test/fixtures/lifecycle-violations/ir-scope-unresolvable-touch', description: '(OI-82) a plan Touches entry resolves to nothing, parent included — check-scope-fence', validator: validatorPath('check-scope-fence.mjs'), expect: 'does not resolve' },
  { id: 'is', dir: 'test/fixtures/lifecycle-violations/is-scope-no-plan', description: '(OI-82) a task has no plan of its own slug to fence against — check-scope-fence', validator: validatorPath('check-scope-fence.mjs'), expect: 'has no corresponding plan in' },
  { id: 'it', dir: 'test/fixtures/lifecycle-violations/it-scope-no-phase-number', description: '(OI-82) a task\'s Active Phase carries no parseable phase number — check-scope-fence', validator: validatorPath('check-scope-fence.mjs'), expect: 'has no parseable phase number' },
  // OI-82 — check-setup-refs (4 of 5). AGENTSMYTH_SETUP_REFS points the map reads at a fixture
  // tree; the schema-absent case additionally needs a definitions root missing one config schema.
  { id: 'iu', dir: 'test/fixtures/setup-refs/empty-maps', description: '(OI-82) a config schema is absent from the definitions root — check-setup-refs', validator: validatorPath('check-setup-refs.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/sr-missing-schema', AGENTSMYTH_SETUP_REFS: 'test/fixtures/setup-refs/empty-maps' }, expect: 'schema not found for domain.yaml' },
  { id: 'iv', dir: 'test/fixtures/setup-refs/no-config-map', description: '(OI-82) the setup references carry no config-map.md — check-setup-refs', validator: validatorPath('check-setup-refs.mjs'), env: { AGENTSMYTH_SETUP_REFS: 'test/fixtures/setup-refs/no-config-map' }, expect: 'no-config-map/config-map.md' },
  { id: 'iw', dir: 'test/fixtures/setup-refs/no-token-map', description: '(OI-82) the setup references carry no token-map.md — check-setup-refs', validator: validatorPath('check-setup-refs.mjs'), env: { AGENTSMYTH_SETUP_REFS: 'test/fixtures/setup-refs/no-token-map' }, expect: 'no-token-map/token-map.md' },
  { id: 'ix', dir: 'test/fixtures/setup-refs/bad-token-field', description: '(OI-82) token-map.md names a field its config schema does not declare — check-setup-refs', validator: validatorPath('check-setup-refs.mjs'), env: { AGENTSMYTH_SETUP_REFS: 'test/fixtures/setup-refs/bad-token-field' }, expect: 'field `domain.nonexistent_field` not found' },
  // OI-82 — check-trigger-predicates (4 of 5). The sandbox fixture path is fixed, so each case
  // diverges the DEFINITIONS side instead: agent-behavior.yaml is copied and mutated one way.
  { id: 'iy', dir: 'test/fixtures/definitions/tp-extra-trigger', description: '(OI-82) agent-behavior declares a trigger the sandbox fixture has no expectation for — check-trigger-predicates', validator: validatorPath('check-trigger-predicates.mjs'), env: { AGENTSMYTH_WF: 'test/fixtures/definitions/tp-extra-trigger' }, expect: 'has no expected outcome for trigger' },
