...
**Note for agents working in this source repo**: source files live under `src/workflow/`;
`workflow/` at the repo root is the dev workspace (config, artifacts, learnings). Consumer
repos receive a compiled `workflow/` install from the bundle — they do not have `src/`.
...
2. `src/workflow/` — canonical workflow rules, lifecycle order, phase contracts, and config.
...
- `src/workflow/lifecycle.md` and the relevant `src/workflow/skills/<phase>/SKILL.md` when entering a phase.
...
- `src/workflow/skills/restore-context/SKILL.md` before resuming an existing slug, recovering after interruption, or processing a user answer to a blocker.
