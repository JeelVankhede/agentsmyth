...
node workflow/validators/check-definitions.mjs
node workflow/validators/check-finding-quality.mjs
node workflow/validators/check-pending-setup.mjs
```
...
- `check-definitions.mjs` validates each definitions file (`agent-behavior.yaml`) against the schema that describes it. Run it against the SOURCE definitions root — in this package, under `AGENTSMYTH_WF` — because the resolved root is otherwise whichever copy the two-root resolver returns, which on a developer machine only moves when `agentsmyth prepare` runs. It fails rather than reporting `ok` when it validated nothing.
- `check-finding-quality.mjs` validates the two-file finding-quality ledger: both files against their schema, rotation in both directions (a row copied rather than moved, a closed row never rotated), a closed-only archive, and a row for every council finding recorded in a council-mode review. Reports the proved-real / noise / pending tally computed over **both** files — a count from the active file alone reports the current cycle rather than the accumulated baseline. Absent ledger is valid unless a council-mode review exists.
- `check-pending-setup.mjs` validates `workflow/config/pending-setup.yaml` item shape and resolution provenance.
- `check-council-record.mjs` validates the **record** a council run leaves behind — a Think council's, recorded in a brief's `## Council Log`, or a Review council's, recorded in a review's. One validator serves both because they are the same contract; two would drift. Table rows are read by header name rather than column position, so the Review record's extra `Risk category`, `Input` and `Status` columns cannot silently shift what every later column means. Checks: presence symmetry between the frontmatter `council:` block and the body `## Council Log`; a `refused` mode carries a refusal reason; council mode carries authorization, resolved cap, `cap_source`, dispatch depth, rounds run, and termination reason; a council-mode record carries a `council.resolution` block, from which the firing decision is re-derived and compared against the recorded mode and refusal reason rather than taken on trust; dispatch depth is 1; per-stage fan-out is within the resolved cap; fan-out is non-increasing across rounds; a round that shrank fan-out follows a round whose `Items closed` cell is non-empty (the open-item delta does not corroborate a taper — items also open mid-run); every round after the first records a sizing rationale; termination is one of `resolved` or `user-decision-required`, a `user-decision-required` run declares its surviving items, and a `resolved` run declaring an item that closed in no round is rejected; every finding is attributed, names a `Round` that exists in the Rounds table, and is attributed to a member declared in Members for that same round; every finding carries a valid evidence class and disposition; `rejected-with-reason` has a non-empty reason; `repo` citations resolve on disk and any line range is within bounds; `trial` citations carry a command and observed output; `web` citations carry URL, retrieval date, and a verbatim quote; two or more non-challenger members sharing a `surface` require a `### Reconcile Contract` that states both how duplicates collapse and how disagreements surface; a `### Conflicts` subsection exists even when empty; a surface holding both accepted and rejected findings has a Conflicts entry; each round containing `web` findings records a challenger spot-check in that same round; a recommendation resting on no `repo` or `trial` finding is rejected — outright when it rests only on `recall`, and when the question's own declared bucket names `repo` as a settling class, since `web` may corroborate a repo-shaped question but not decide it (such a question must name its bucket, and a bucket reference resolving to no classification row is itself rejected); and every finding's class has a recorded availability status.

  **Review records additionally require:** every member declares its input, and no input names the Build session transcript — a reviewer that reads the author's reasoning reviews the intention rather than the change; reviewer risk categories are partitioned disjointly, since two reviewers holding one category means another went unread; a member recorded `failed` has a `### Skipped Checks` entry carrying all six fields `verification.yaml` requires; `council.repo_integrity` is present and unchanged whether or not any member declared a sandbox, because a Review council reads the repository whose changes it is judging; and the council-log Findings table declares no fix-recommendation column, because proposing a fix switches a read-only candidate to Build scope. Emits a summary line on success — rounds, findings, rejections, uncorroborated `recall`, and citations mechanically resolved versus shape-checked only.
  ...
  **What `check-council-record.mjs` does not check.** Stated plainly, because a validator this specific invites the assumption that it guarantees more than it does:
  ...
  - whether a fix recommendation was **smuggled into a reason field**. The no-fix rule is enforced structurally, against a declared column. Scanning reason prose for imperative phrasing was rejected deliberately: this repo has twice shipped a keyword matched without regard to its clause — a coverage cell reading "never silently dropped" read as a drop claim, a waiver cell reading "rather than a waiver" read as a waiver — and a third would cost more in false rejections than the rule gains.
