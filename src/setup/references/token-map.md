...
| Token | Config | Field |
|---|---|---|
| `{{REPO_NAME}}` | `workflow/config/domain.yaml` | `domain.name` |
...
| `{{VERIFICATION_CMDS}}` | `workflow/config/verification.yaml` | `commands[].command` (array) |
| `{{CONSTRAINTS}}` | `workflow/config/domain.yaml` | `constraints.product[]`, `constraints.safety[]` (arrays) |
