...
**Post-Test resolution (added by Ship, not a rewrite of this phase's own findings):** both `waiver-required` rows above are now closed. The real-CI-run gap is resolved, not waived — PR #41 (`feat/wp-r11-docs-site` → `main`) triggered a genuine GitHub Actions `validate` run, which passed on the second attempt after a real bug it caught (a `check-waivers.mjs` false-positive in a task artifact's prose, fixed by rewording, not by bypassing the check). The `npm audit` gap is waived by explicit user decision. Full detail, evidence, and the formal waiver record are in `workflow/artifacts/ship/wp-r11-docs-site-v1.md`; this row is left as Test originally wrote it, since Test's own recommendation was correct given what was knowable at Test time.
## Architecture Notes
