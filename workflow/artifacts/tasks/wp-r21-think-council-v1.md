...
- `test/fixtures/lifecycle-violations/dm-resolved-with-open-items` — new; `resolved` while the final round still records open items — IDs: R13, RI9
- `test/fixtures/lifecycle-violations/dn-escalation-survivors-none` — new; an escalation whose surviving-items line names no ID — IDs: R13, RI9
...
| Phase 7 - Rejection fixtures (third external review pass) | 2026-08-29 | Third pass approved the branch and left four items, all fixed here: `resolved` is now corroborated by the final round's `Open out` and `user-decision-required` must name at least one survivor — the two paths left open when `max-rounds` was removed — plus `r21-termination-enum` rewritten to compare the validator's `TERMINATIONS` against the schema's `termination_reason.enum` directly instead of pinning superseded sentences, and the Exit Gate's stale bullet count replaced with a count-free reference. Nothing deferred. violations 67/67 → **69/69**, conformance 26/26, `validate` exit 0, attribution sweep exactly one error per council fixture |
