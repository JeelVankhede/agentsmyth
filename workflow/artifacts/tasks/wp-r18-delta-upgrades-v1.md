...
- Phase: Phase 13 - Test finding remediation (BUILD COMPLETE - 13 phases)
...
| Phase 13 - Test finding remediation | complete | RI3, RI14, RI16 |
...
|---|---|---|---|
| Phase 13 - Test finding remediation | complete | 2026-09-24 | Test returned `hold` on two findings, both defects in Phase 12's own remediation and both the same shape: a check that cannot fail for the reason it was written. **V1** - the RI14 enforcement required EVERY governed file to differ, but setup rewrites the five configs and never touches the hook or the Cursor adapter, so the real signature is five drifted of seven and the condition was dead. Scoped to config entries. While fixing it the same mistake recurred one level down: the first scoping keyed on `wf`, the validator's overridable notion of the workflow directory, while entry paths come from the CLI which always writes `workflow/config/` - under `AGENTSMYTH_WF`, which is how every fixture runs, those disagree and the rule silently stopped firing, taking its own new rejection fixture with it. Caught by `violations:test` dropping to 222/223, not by reading the code. Now matched on the manifest's own path shape. **V2** - the AGENTS.md installed-version read probed for a package.json relative to the validator, which resolves in `src/` and in no location a consumer runs from, so the comparison was inert everywhere it shipped while its unit test stayed green. `prepare` now stamps `installed-version.txt` into the global tree it expands, and the validator reads that first. The test runs the validator FROM the global tree, which is the whole point. The revert probe initially called this fix unpinned because it reverts `src/` while the global tree expands from `dist/` - the identical wrong-path error, in the tooling this time; the probe now rebuilds the bundle before running. Both findings re-verified by the probes that found them rather than by new assertions written to agree with the fix. Suite 117 -> 127. |
